...
# Probable pitchers — hoy Y mañana
print("-> Probable pitchers (hoy + mañana)...")
probable_pitchers = {}      # hoy
probable_tomorrow = {}      # mañana

TOMORROW = TODAY + datetime.timedelta(days=1)

def parse_probable_from_page(date):
    result = {}
    ...
    for row in table.find_all('tr')[1:]:
        ...
        if len(cells) >= 2:
            ...
            if home: result[home] = {'pitcher': hpitch or 'TBD', 'isHome': True,  'date': date.isoformat()}
            if away: result[away] = {'pitcher': apitch or 'TBD', 'isHome': False, 'date': date.isoformat()}
    return result

probable_pitchers = parse_probable_from_page(TODAY)
print(f"  hoy ({TODAY}): {len(probable_pitchers)} equipos")

probable_tomorrow = parse_probable_from_page(TOMORROW)
print(f"  mañana ({TOMORROW}): {len(probable_tomorrow)} equipos")

# Si hoy no hay partidos (día de descanso), usar mañana como principal
if not probable_pitchers and probable_tomorrow:
    print("  -> Sin partidos hoy, usando mañana como principal")
    probable_pitchers = probable_tomorrow
    probable_tomorrow = {}
...
output = {
    ...
    "npb_probable_tomorrow": probable_tomorrow,
}
