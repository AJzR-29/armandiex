...
def natstat_get(endpoint, service, range_params='', extra_params={}):
    ...
    try:
        ...
        if r.status_code == 500:
            ...
            p2 = dict(p)
            if range_params:
                parts = [p.strip() for p in range_params.split(',')]
                for part in parts:
                    if part.isdigit() and len(part)==4:
                        p2['season'] = part
                    elif len(part)==10 and part[4]=='-':
                        p2['date'] = part
                    elif part in ('player','team'):
                        p2['type'] = part
                    elif part in ('era','avg','whip','ops','hr','rbi','k','bb','ip','fip'):
                        p2['stat'] = part
                    elif part:
                        p2['search'] = part
        ...
        if isinstance(data, dict):
            ...
            # Show first non-meta key content preview
            for k in keys:
                if k not in ('meta','success') and data[k]:
                    val = data[k]
                    if isinstance(val, (dict,list)):
                        first = list(val.values())[0] if isinstance(val,dict) else val[0]
                        print(f"    [{k}] sample: {str(first)[:150]}")
                    break
        return data
...
npb_today = parse_games(
    natstat_get('games', 'npb', '', {'date':TODAY.strftime('%Y-%m-%d')}),
    TODAY.isoformat()
)
...
npb_forecasts = parse_forecasts(
    natstat_get('forecasts', 'npb', '', {'date':TODAY.strftime('%Y-%m-%d')})
)
...
npb_pitching = parse_stats(natstat_get('stats', 'npb', '', {'season':SEASON,'type':'player','stat':'era'}))
print(f"   {len(npb_pitching)} pitchers")
time.sleep(0.4)

print("→ NPB batting (AVG, season)...")
npb_batting = parse_stats(natstat_get('stats', 'npb', '', {'season':SEASON,'type':'player','stat':'avg'}))
print(f"   {len(npb_batting)} batters")
time.sleep(0.4)

print("→ NPB team batting...")
npb_team_bat = parse_stats(natstat_get('stats', 'npb', '', {'season':SEASON,'type':'team','stat':'avg'}))
print(f"   {len(npb_team_bat)} teams")
time.sleep(0.4)

print("→ NPB team pitching...")
npb_team_pit = parse_stats(natstat_get('stats', 'npb', '', {'season':SEASON,'type':'team','stat':'era'}))
...
cbb_today = parse_games(
    natstat_get('games', 'cbb', '', {'date':TODAY.strftime('%Y-%m-%d')}),
    TODAY.isoformat()
)
...
cbb_forecasts = parse_forecasts(
    natstat_get('forecasts', 'cbb', '', {'date':TODAY.strftime('%Y-%m-%d')})
)
...
print("→ CBB pitching...")
cbb_pitching = parse_stats(natstat_get('stats', 'cbb', '', {'season':SEASON,'type':'player','stat':'era'}))
...
print("→ CBB batting...")
cbb_batting = parse_stats(natstat_get('stats', 'cbb', '', {'season':SEASON,'type':'player','stat':'avg'}))
...
cbb_team_bat = parse_stats(natstat_get('stats', 'cbb', '', {'season':SEASON,'type':'team','stat':'avg'}))
cbb_team_pit = parse_stats(natstat_get('stats', 'cbb', '', {'season':SEASON,'type':'team','stat':'era'}))
cbb_team_stats = []
...
mlb_forecasts = parse_forecasts(
    natstat_get('forecasts', 'mlb', '', {'date':TODAY.strftime('%Y-%m-%d')})
)
...
mlb_moneyline = parse_forecasts(
    natstat_get('moneyline', 'mlb', '', {'date':TODAY.strftime('%Y-%m-%d')})
)
