document.addEventListener('DOMContentLoaded', function () {
  ...
  function parsePitcherStats(statsArr){
    ...
    (statsArr||[]).forEach(function(s){
      ...
      if(grp==='pitching'&&typ==='statsSingleSeason'){
        var st=s.stats||{};
        ps.era=st.era||'—'; ps.wins=parseInt(st.wins)||0; ps.losses=parseInt(st.losses)||0;
        ps.ks=parseInt(st.strikeOuts)||0; ps.starts=parseInt(st.gamesStarted)||0;
        ps.whip=st.whip||'—'; ps.ip=parseFloat(st.inningsPitched)||0;
        var bb=parseInt(st.baseOnBalls)||0; var hr=parseInt(st.homeRuns)||0;
        if(ps.ip>0){
  ...
  async function fetchPitcherStatsById(pitcherId){
    try{
      ...
      return parsePitcherStats(person.stats||[]);
    }catch(e){return null;}
  }
        ...
        (d.games||[]).forEach(function(g){
          ...
          ['home','away'].forEach(function(side){
            ...
            if(t.probablePitcher){
              ...
              // Intentar parsear stats del hydrate inline
              var ps=parsePitcherStats(p.stats||[]);
            ...
            map[t.team.name]={
              pitcher:pitcher,wins:rec.wins||0,losses:rec.losses||0,teamId:t.team.id,
              isHome:isHome,isDayGame:isDayGame,gameId:g.gamePk
            };
          });
        });
      });
      // Segunda pasada: para pitchers con ERA='—', fetch directo por ID
      var toFetch=[];
      Object.values(map).forEach(function(v){
        if(v.pitcher&&v.pitcher.id&&v.pitcher.era==='—'){
          toFetch.push({teamName:Object.keys(map).find(function(k){return map[k]===v;}),pitcher:v.pitcher});
        }
      });
      if(toFetch.length){
        var results=await Promise.all(toFetch.map(function(item){
