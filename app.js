document.addEventListener('DOMContentLoaded', function () {
  ...
  async function buildGameCard(ev,oddsData,mlbSched,mlbHit,isMLB){
    ...
    // ── Fetch paralelo: lineup + team stats + pitcher fatigue ──
    var [lineup,homeRecent,awayRecent,homeFatigue,awayFatigue]=await Promise.all([
    ...
    // Store props data on the div for lazy rendering
    div._propsData={homeTeam:homeTeam,awayTeam:awayTeam,hp:hp,ap:ap,
  ...
  function calcHitProb(batter, pitcherK9, ballparkFactor){
    // Base: AVG reciente o de temporada
    var avg = parseFloat(batter.avg);
    if(isNaN(avg) || avg <= 0) avg = 0.250; // league avg fallback

    // P(1+ hit en N ABs) = 1 - (1-avg)^N   con N esperado ~3.8 ABs
    var expectedAB = 3.8;

    // Ajuste por K/9 del pitcher (más Ks → menos contacto)
    var k9 = parseFloat(pitcherK9);
    var pitcherAdj = !isNaN(k9)
      ? (k9 >= 10 ? -0.025 : k9 >= 8 ? -0.012 : k9 <= 5 ? +0.015 : 0)
      : 0;

    // Ajuste ballpark
    var bpAdj = ballparkFactor ? (ballparkFactor - 1) * 0.04 : 0;

    // xBA ajuste si existe
    var xba = parseFloat(batter.xba);
    if(!isNaN(xba) && xba > 0) avg = avg * 0.55 + xba * 0.45;

    var adjAvg = Math.max(0.10, Math.min(0.65, avg + pitcherAdj + bpAdj));
    var prob   = 1 - Math.pow(1 - adjAvg, expectedAB);
    return Math.round(prob * 100);
  }

  function calcKProb(pitcher, lineupAgg, inningsExpected){
    // Base: K/9 del pitcher
    var k9 = parseFloat(pitcher.k9);
    if(isNaN(k9) || k9 <= 0){
      // Fallback desde ERA/FIP
      var era = parseFloat(pitcher.era);
      k9 = !isNaN(era) ? Math.max(4, 12 - era * 0.8) : 7.5;
    }

    // Innings esperados (titular típico: 5-6)
    var ip = inningsExpected || 5.5;

    // K esperados = K/9 × IP / 9
    var expectedK = k9 * ip / 9;

    // Ajuste por BB% del lineup contrario (más BB → más pitches → menos IP)
    ...
    // P(5+ Ks) usando aproximación Poisson
    // P(X>=5) = 1 - P(X<=4)
    ...
    var lambda = expectedK;
    var pLess5 = 0;
    for(var k=0;k<=4;k++) pLess5 += poissonPMF(lambda, k);
    var prob = Math.round((1 - pLess5) * 100);

    return {prob: Math.max(5, Math.min(95, prob)), expectedK: expectedK.toFixed(1)};
  }
  ...
  function toggleProps(btn, uid){
    ...
    if(!panel.innerHTML){
      var card=btn.closest('div[style*="border-radius:14px"]');
      if(!card||!card._propsData){panel.innerHTML='<div style="color:#555;padding:8px">Sin datos</div>';panel.style.display='block';return;}
      var d=card._propsData;
      panel.innerHTML=renderPropsPanel(
  ...
  function renderPropsPanel(homeTeam, awayTeam, hp, ap,
    ...
    // ── Hits props ───────────────────────────────────────────────
    function buildHitProps(batters, pitcherRival, label){
      ...
      var rows = batters.map(function(b){
        var prob = calcHitProb(b, pitcherRival.k9, bf);
    ...
    var html =
      '<div style="background:#0a0a18;border:1px solid #7f5af044;border-radius:12px;padding:14px;margin-top:8px">'+
        ...
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">'+
          '<div>'+
            buildHitProps(homeBatters, ap, '🏠 '+homeTeam.split(' ').slice(-1)[0]+' vs '+ap.name.split(' ').slice(-1)[0])+
          '</div>'+
          '<div>'+
            buildHitProps(awayBatters, hp, '✈️ '+awayTeam.split(' ').slice(-1)[0]+' vs '+hp.name.split(' ').slice(-1)[0])+
          '</div>'+
        '</div>'+
        '<div style="margin-top:10px;padding-top:8px;border-top:1px solid #1a1a2e">'+
          ...
          '<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">'+
            '<div>'+buildKProp(hp, awayLineupAgg, '🏠 Local vs lineup visita')+'</div>'+
            '<div>'+buildKProp(ap, homeLineupAgg, '✈️ Visita vs lineup local')+'</div>'+
          '</div>'+
        '</div>'+
