...
document.addEventListener('DOMContentLoaded', function () {
  ...
  async function fetchInjuries(teamId){
    ...
    try{
      ...
      (data.roster||[]).forEach(function(r){
        ...
        // Only count hitters — exclude pitchers and two-way players
        var isHitter=OFFENSIVE_POS.indexOf(pos)>=0||
                     (posType==='Hitter')||(posType==='Catcher')||(posType==='Infielder')||(posType==='Outfielder');
        var isPitcher=pos==='P'||posType==='Pitcher';
        if(isHitter&&!isPitcher){
          injured.push({name:p.fullName||'—',pos:pos,status:r.status||'IL'});
        }
      });
      // Sanity cap — more than 8 hitters on IL is unusual, likely a data issue
      if(injured.length>8) injured=injured.slice(0,8);
  ...
  async function buildGameCard(ev,oddsData,mlbSched,mlbHit,isMLB,leagueKey){
    ...
    if(homeInjuries&&homeInjuries.length>=3) alerts.push('🤕 '+homeAbbr+' '+homeInjuries.length+' bateadores en IL');
    else if(homeInjuries&&homeInjuries.length>=1) alerts.push('🤕 '+homeAbbr+': '+homeInjuries.map(function(i){return i.pos;}).join(', ')+' en IL');
    if(awayInjuries&&awayInjuries.length>=3) alerts.push('🤕 '+awayAbbr+' '+awayInjuries.length+' bateadores en IL');
    else if(awayInjuries&&awayInjuries.length>=1) alerts.push('🤕 '+awayAbbr+': '+awayInjuries.map(function(i){return i.pos;}).join(', ')+' en IL');
