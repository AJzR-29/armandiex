...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var oddsCache          = {};   // key=leagueKey, value={data,ts}
  var ODDS_TTL           = 30 * 60 * 1000; // 30 min in ms
  ...
  // ── New v4 data helpers ────────────────────────────────────────────────
  function findPitcherWL(name){
    // pitcher_wl from MLB-StatsAPI league_leaders — real W-L, no delay
    if(!mlbStatsJSON||!mlbStatsJSON.pitcher_wl) return null;
    var n=name.toLowerCase().split(' ').pop();
    var entry=Object.values(mlbStatsJSON.pitcher_wl).find(function(p){
      return p.name&&p.name.toLowerCase().includes(n);
    });
    return entry||null;
  }

  function findStandings(teamName){
    if(!mlbStatsJSON||!mlbStatsJSON.standings) return null;
    var n=teamName.toLowerCase().split(' ').pop();
    return mlbStatsJSON.standings.find(function(s){
      return s.team&&s.team.toLowerCase().includes(n);
    })||null;
  }

  function findFirst5(teamName){
    if(!mlbStatsJSON||!mlbStatsJSON.first5_stats) return null;
    var n=teamName.toLowerCase().split(' ').pop();
    return mlbStatsJSON.first5_stats.find(function(s){
      return s.team&&s.team.toLowerCase().includes(n);
    })||null;
  }
  function findBettingSplits(homeTeam, awayTeam){
    if(!mlbStatsJSON||!mlbStatsJSON.betting_splits) return null;
    var hn=homeTeam.toLowerCase().split(' ').pop();
    var an=awayTeam.toLowerCase().split(' ').pop();
    return mlbStatsJSON.betting_splits.find(function(g){
      return (g.home&&g.home.toLowerCase().includes(hn)&&g.away&&g.away.toLowerCase().includes(an))||
             (g.home&&g.home.toLowerCase().includes(an)&&g.away&&g.away.toLowerCase().includes(hn));
    })||null;
  }

  function findPitcherL5(pitcherId){
    if(!mlbStatsJSON||!mlbStatsJSON.pitcher_l5) return null;
    return mlbStatsJSON.pitcher_l5[String(pitcherId)]||null;
  }

  function findF5Odds(homeTeam, awayTeam){
    if(!mlbStatsJSON||!mlbStatsJSON.f5_odds) return null;
    var hn=homeTeam.toLowerCase().split(' ').pop();
    var an=awayTeam.toLowerCase().split(' ').pop();
    return mlbStatsJSON.f5_odds.find(function(g){
      return (g.home&&g.home.toLowerCase().includes(hn)&&g.away&&g.away.toLowerCase().includes(an))||
             (g.home&&g.home.toLowerCase().includes(an)&&g.away&&g.away.toLowerCase().includes(hn));
    })||null;
  }
  ...
  // ── F5 Model ──────────────────────────────────────────────────────────
  function calcF5Prediction(homeTeam, awayTeam, hp, ap, hpL5, apL5, f5OddsData){
    var LEAGUE_AVG_ERA = 4.20;
    var LEAGUE_AVG_IP  = 5.5;  // avg innings pitched per start

    function pitcherF5Score(p, l5){
      if(!p||p.name==='TBD') return 0.45;
      // Use L5 ERA if available, fallback to season ERA
      var era = (l5&&l5.era_l5) ? l5.era_l5 : parseFloat(p.era)||LEAGUE_AVG_ERA;
      var avgIp = (l5&&l5.avg_ip) ? l5.avg_ip : LEAGUE_AVG_IP;
      // Pitcher who goes deep into games is better F5 bet
      var eraScore = clamp((7.0 - era) / 5.0, 0, 1);
      var ipScore  = clamp((avgIp - 3.0) / 3.5, 0, 1);
      return clamp(eraScore*0.65 + ipScore*0.35, 0, 1);
    }

    function teamF5OffScore(teamName){
      var f5 = findFirst5(teamName);
      if(!f5) return 0.45;
      // avg_scored_f5 > 2.8 is above average
      var offScore = clamp((f5.avg_scored_f5 - 1.0) / 3.5, 0, 1);
      return clamp(offScore, 0, 1);
    }

    var hPitch = pitcherF5Score(hp, hpL5);
    var aPitch = pitcherF5Score(ap, apL5);
    var hOff   = teamF5OffScore(homeTeam);
    var aOff   = teamF5OffScore(awayTeam);

    // Home advantage smaller in F5 (no bullpen factor)
    var homeAdv = 0.018;

    var homeOwn = (hOff*0.45 + (1-aPitch)*0.55) + homeAdv;
    var awayOwn = (aOff*0.45 + (1-hPitch)*0.55);
    ...
    var f5MktHome = 0.5;
    if(f5OddsData){
      var hi = f5OddsData.f5_home_impl||50;
      var ai = f5OddsData.f5_away_impl||50;
      f5MktHome = clamp(hi/(hi+ai), 0.05, 0.95);
    }

    var hasMkt = f5OddsData && f5OddsData.f5_books_count >= 2;
    var hasL5  = !!(hpL5&&apL5);
    var ratio  = hasMkt ? (hasL5?{own:0.65,mkt:0.35}:{own:0.50,mkt:0.50})
                        : {own:0.85,mkt:0.15};

    var finalHome = clamp(ownHome*ratio.own + f5MktHome*ratio.mkt, 0.05, 0.95);

    return {
      f5ModelHome: finalHome,
      f5ModelAway: 1 - finalHome,
      f5OwnHome:   ownHome,
      f5MktHome:   f5MktHome,
      hPitch: hPitch, aPitch: aPitch,
      hOff: hOff, aOff: aOff,
      hEraL5: hpL5&&hpL5.era_l5||null,
      aEraL5: apL5&&apL5.era_l5||null,
      hAvgIp: hpL5&&hpL5.avg_ip||null,
      aAvgIp: apL5&&apL5.avg_ip||null,
    };
  }
  ...
  function calcNPBPrediction(homeTeam, awayTeam, homeData, awayData, mktHome, mktAway, wind, stadium){
    ...
    function npbPitcherScore(pitcher){
      ...
      var fg=findNatstatNPBPitcher(pitcher.name)||findNPBPitcher(pitcher.name);
      if(!fg) return 0.45;
    ...
    // Dynamic ratio + Natstat ELO
    var _npbHasPitcher=!!(homeData&&homeData.pitcher&&homeData.pitcher.name!=='TBD');
    var _npbName=(homeData&&homeData.pitcher&&homeData.pitcher.name)||'';
    var _npbHasFG=!!(findNatstatNPBPitcher(_npbName)||findNPBPitcher(_npbName));
    var _npbR=dynamicRatio(_npbHasPitcher,_npbHasFG,mktHome!==0.5,false);
    var hNPBelo=getNatstatELO('NPB',homeTeam);
    var aNPBelo=getNatstatELO('NPB',awayTeam);
    var eloNPB=0.5;
    if(hNPBelo&&hNPBelo.win_prob) eloNPB=clamp(hNPBelo.win_prob,0.1,0.9);
    else if(hNPBelo&&hNPBelo.elo&&aNPBelo&&aNPBelo.elo)
      eloNPB=clamp(1/(1+Math.pow(10,-(hNPBelo.elo-aNPBelo.elo)/400)),0.1,0.9);
    var finalHome=hNPBelo
      ?clamp(ownHome*0.50+eloNPB*0.30+mktHome*0.20,0.05,0.95)
      :clamp(ownHome*_npbR.own+mktHome*_npbR.mkt,0.05,0.95);
  ...
  async function buildNPBGameFromData(game,oddsData){
    ...
    if(oddsEv&&absEdge>=0.05&&bkCount>=2&&!windBad){
      ...
      recC='rec-avoid';
      recL='🔴 Parejo · Ventaja leve: '+modelFav+' '+modelFavPct+'% — evitar apostar';
    }
    ...
    if(sharpSignal){
      if(sharpSignal.indexOf(betTeam)>=0||sharpSignal.indexOf(betTeam.split(' ').pop())>=0)
        recL+=' · ⚡ Sharp confirma';
      else recL+=' · ⚠️ Sharp en contra';
    }
    ...
    div.innerHTML=
      ...
      // ── EV Panel ────────────────────────────────────────────
      (bestEV!==null?(
        '<div style="background:var(--surface2);border:1px solid '+(bestEV>0.05?'#10b98133':bestEV>0?'#f59e0b33':'#ef444433')+';border-radius:10px;padding:12px;margin-top:4px">'+
          '<div style="font-family:DM Mono,monospace;font-size:0.65rem;color:var(--muted);text-transform:uppercase;letter-spacing:0.08em;margin-bottom:8px">⚡ Expected Value</div>'+
          '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:8px">'+
            '<div class="mlb-odd-box">'+
              '<div class="mlb-odd-label">EV por $100</div>'+
              '<div class="mlb-odd-value" style="color:'+(bestEV>0.05?'#10b981':bestEV>0?'#f59e0b':'#ef4444')+'">'+
                (bestEV>0?'+':'')+( bestEV*100).toFixed(1)+'</div>'+
            '</div>'+
            '<div class="mlb-odd-box">'+
              '<div class="mlb-odd-label">Cuota disponible</div>'+
              '<div class="mlb-odd-value" style="color:var(--text)">'+bestEVodd.toFixed(2)+'</div>'+
            '</div>'+
            '<div class="mlb-odd-box">'+
              '<div class="mlb-odd-label">Cuota mínima</div>'+
              '<div class="mlb-odd-value" style="color:'+(bestEVodd>=(betH?minOddHome:minOddAway)?'#10b981':'#ef4444')+'">'+
                (betH?minOddHome:minOddAway)+'</div>'+
            '</div>'+
          '</div>'+
          '<div style="font-size:0.72rem;font-weight:600;color:'+(bestEV>0.05?'#10b981':bestEV>0?'#f59e0b':'#ef4444')+';text-align:center">'+
            (bestEV>0.10?'🔥 EV excelente — apuesta recomendada':
             bestEV>0.05?'✅ EV positivo — vale la pena':
             bestEV>0?'🟡 EV marginal — proceder con cautela':
             '❌ EV negativo — evitar')+
          '</div>'+
        '</div>'
      ):'')+
      ...
      (valueSigns.length>0?(
        '<div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;padding:12px;margin-top:4px">'+
          '<div style="font-family:DM Mono,monospace;font-size:0.65rem;color:var(--muted);text-transform:uppercase;letter-spacing:0.08em;margin-bottom:8px">🔍 Señales de valor</div>'+
          valueSigns.map(function(s){
            var c=s.positive===true?'#10b981':s.positive===false?'#ef4444':'#f59e0b';
            return '<div style="display:flex;justify-content:space-between;align-items:center;padding:4px 0;border-bottom:1px solid var(--border);font-size:0.76rem">'+
              '<span>'+s.icon+' <span style="color:var(--text)">'+s.text+'</span></span>'+
              '<span style="color:'+c+';font-family:DM Mono,monospace;font-size:0.68rem">'+s.detail+'</span>'+
            '</div>';
          }).join('')+
        '</div>'
      ):'')+
  ...
  async function loadNPBGamesPanel(){
    ...
    try{
      ...
      // Natstat is primary source — npb_stats.json is fallback
      var todayGames=(natstatJSON&&natstatJSON.npb_games_today&&natstatJSON.npb_games_today.length)
        ?natstatJSON.npb_games_today
        :(npbJSON&&npbJSON.npb_games_today)||[];
      var tomorrowGames=(natstatJSON&&natstatJSON.npb_games_tomorrow&&natstatJSON.npb_games_tomorrow.length)
        ?natstatJSON.npb_games_tomorrow
        :(npbJSON&&npbJSON.npb_games_tomorrow)||[];

      // Last fallback: build from probable_pitchers
  ...
  function calcNCAAPrediction(homeTeam, awayTeam, hPitcher, aPitcher, mktHome, mktAway){
    ...
    function ncaaPitcherScore(pitcherName){
      ...
      // Try Natstat CBB first, then ncaa_stats.json fallback
      var p=findNatstatCBBPitcher(pitcherName)||findNCAAPitcher(pitcherName);
      if(!p) return 0.45;
    ...
    function ncaaOffenseScore(teamName){
      var t=findNatstatCBBTeam(teamName)||findNCAATeamStats(teamName);
      if(!t) return 0.45;
    ...
    // Dynamic ratio + Natstat ELO + RPI
    var _ncaaHasPitcher=!!(hPitcher&&hPitcher!=='TBD'&&aPitcher&&aPitcher!=='TBD');
    var _ncaaHasFG=!!(findNatstatCBBPitcher(hPitcher||'')||findNCAAPitcher(hPitcher||'')||
                      findNatstatCBBPitcher(aPitcher||'')||findNCAAPitcher(aPitcher||''));
    var _ncaaR=dynamicRatio(_ncaaHasPitcher,_ncaaHasFG,mktHome!==0.5,false);
    var hCBBelo=getNatstatELO('CBB',homeTeam);
    var aCBBelo=getNatstatELO('CBB',awayTeam);
    var eloCBB=0.5;
    if(hCBBelo&&hCBBelo.win_prob) eloCBB=clamp(hCBBelo.win_prob,0.1,0.9);
    else if(hCBBelo&&hCBBelo.elo&&aCBBelo&&aCBBelo.elo)
      eloCBB=clamp(1/(1+Math.pow(10,-(hCBBelo.elo-aCBBelo.elo)/400)),0.1,0.9);
    var hTeamNat=findNatstatCBBTeam(homeTeam);
    var aTeamNat=findNatstatCBBTeam(awayTeam);
    var rpiAdj=0;
    if(hTeamNat&&aTeamNat&&hTeamNat.rpi&&aTeamNat.rpi)
      rpiAdj=clamp((parseFloat(hTeamNat.rpi)-parseFloat(aTeamNat.rpi))*0.001,-0.05,0.05);
    var finalHome=hCBBelo
      ?clamp(ownHome*0.45+eloCBB*0.35+mktHome*0.20+rpiAdj,0.05,0.95)
      :clamp(ownHome*_ncaaR.own+mktHome*_ncaaR.mkt+rpiAdj,0.05,0.95);
  ...
  // ── Natstat JSON loader ─────────────────────────────────────────────
  async function loadNatstatJSON(){
    if(natstatJSON) return natstatJSON;
    try{
      var res=await fetch('natstat_stats.json');
      if(!res.ok) throw new Error('natstat_stats.json not found');
      natstatJSON=await res.json();
      console.log('[natstat] loaded:', natstatJSON.generated_at,
        '| NPB:', (natstatJSON.npb_games_today||[]).length,
        '| CBB:', (natstatJSON.cbb_games_today||[]).length);
    }catch(e){ console.warn('[natstat]',e.message); natstatJSON={}; }
    return natstatJSON;
  }

  function findNatstatNPBPitcher(name){
    if(!natstatJSON||!natstatJSON.npb_pitching) return null;
    var n=(name||'').toLowerCase().split(' ').pop();
    return natstatJSON.npb_pitching.find(function(p){return p.name&&p.name.toLowerCase().includes(n);})||null;
  }

  function findNatstatCBBPitcher(name){
    if(!natstatJSON||!natstatJSON.cbb_pitching) return null;
    var n=(name||'').toLowerCase().split(' ').pop();
    return natstatJSON.cbb_pitching.find(function(p){return p.name&&p.name.toLowerCase().includes(n);})||null;
  }

  function findNatstatCBBTeam(teamName){
    if(!natstatJSON||!natstatJSON.cbb_team_stats) return null;
    var n=(teamName||'').toLowerCase().split(' ').pop();
    return natstatJSON.cbb_team_stats.find(function(t){return t.team&&t.team.toLowerCase().includes(n);})||null;
  }

  function getNatstatELO(league, teamName){
    var map=league==='NPB'?(natstatJSON&&natstatJSON.npb_elo):
            league==='CBB'?(natstatJSON&&natstatJSON.cbb_elo):
            (natstatJSON&&natstatJSON.mlb_elo);
    if(!map) return null;
    var n=(teamName||'').toLowerCase().split(' ').pop();
    var key=Object.keys(map).find(function(k){return k.toLowerCase().includes(n);});
  ...
  async function fetchPitcherStatsById(pitcherId){
    try{
      ...
      // Get real W-L — try pitcher_wl from JSON first (most accurate, no delay)
      var _wlJSON=findPitcherWL(person.fullName||'');
      if(_wlJSON){
        ps.wins=_wlJSON.w||0;
        ps.losses=_wlJSON.l||0;
      } else if(logRes.ok){
        // Fallback: gameLog
        try{
          var logData=await logRes.json();
          ...
          if(splits.length>0){ ps.wins=wins; ps.losses=losses; if(starts>0) ps.starts=starts; }
        }catch(e){}
      }
  ...
  async function fetchMLBLineup(gameId){
    ...
    try{
      // Fetch boxscore (official) + probable lineups in parallel
      var [boxRes, probRes] = await Promise.all([
        fetch(MLBAPI+'/game/'+gameId+'/boxscore'),
        fetch(MLBAPI+'/schedule?gamePk='+gameId+'&hydrate=lineups')
      ]);
      if(!boxRes.ok) return null;
      var data=await boxRes.json();
      var result={home:[],away:[],homeConfirmed:false,awayConfirmed:false,
                  homeProbable:[],awayProbable:[],isProbable:false};

      // Try probable lineups from schedule hydrate
      if(probRes.ok){
        try{
          var probData=await probRes.json();
          var game=(probData.dates&&probData.dates[0]&&probData.dates[0].games&&probData.dates[0].games[0])||null;
          var lineups=game&&game.lineups;
          if(lineups){
            ['homePlayers','awayPlayers'].forEach(function(side){
              var players=lineups[side]||[];
              var key=side==='homePlayers'?'homeProbable':'awayProbable';
              players.forEach(function(pl){
                var s=pl.seasonStats&&pl.seasonStats.batting||{};
                result[key].push({
                  name:(pl.fullName||pl.person&&pl.person.fullName)||'—',
                  avg:s.avg||'—', obp:s.obp||'—',
                  slg:s.slg||'—', ops:s.ops||'—'
                });
              });
            });
            if(result.homeProbable.length>0||result.awayProbable.length>0){
              result.isProbable=true;
              console.log('[lineup] probable lineups found for game',gameId);
            }
          }
        }catch(e){ console.warn('[lineup probable]',e); }
      }
      ['home','away'].forEach(function(side){
        ...
        if(battingOrder.length>=9){
          result[side+'Confirmed']=true;
          result.isProbable=false; // official overrides probable
        }
  ...
  async function loadMLBGamesPanel(leagueKey){
    ...
    try{
      ...
      // Use cached odds if < 30 min old — prevents result flip on refresh
      var _now=Date.now();
      var _cached=oddsCache[leagueKey];
      var fetchOdds=(_cached&&(_now-_cached.ts)<ODDS_TTL)
        ?Promise.resolve({ok:true,_fromCache:true,_data:_cached.data,headers:{get:function(){return null;}}})
      ...
      if(isNCAA){ await loadNCAATatsJSON(); await loadNatstatJSON(); }
      ...
      if(rs[1]&&rs[1].ok){
        if(rs[1]._fromCache){
          oddsData=rs[1]._data||[];
          quotaInfo.textContent='Odds en caché · sin créditos usados';
        } else {
          var _od2=await rs[1].json(); if(Array.isArray(_od2)) oddsData=_od2;
          oddsCache[leagueKey]={data:oddsData,ts:Date.now()};
  ...
  async function buildGameCard(ev,oddsData,mlbSched,mlbHit,isMLB,leagueKey){
    ...
    if(leagueKey==='baseball_ncaa'){
      ...
      if(oddsEv&&ncaaAbsEdge>=0.05&&bkCount>=2){
        ...
        ncaaRecC='rec-avoid';
        ncaaRecL='🔴 Parejo · Ventaja leve: '+ncaaFav+' '+ncaaFavPct+'% — evitar apostar';
      }
      var ncaaBreakdown=
        '<div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:6px;margin-top:8px">'+
    ...
    // ── Run differential adjustment (from standings) ──────────
    // Run diff is a better predictor of true talent than W-L
    var hStandings=findStandings(homeTeam);
    var aStandings=findStandings(awayTeam);
    var hRunDiff=hStandings&&hStandings.run_diff!=null?hStandings.run_diff:0;
    var aRunDiff=aStandings&&aStandings.run_diff!=null?aStandings.run_diff:0;
    // Scale: every 30 run differential = ~1% adj (max ±3%)
    var hRDadj=Math.max(-0.03,Math.min(0.03,(hRunDiff-aRunDiff)/3000));

    // ── Score propio por equipo ───────────────────────────────
    var homeOwnScore = (hOffScore*0.50 + (1-aPitchScore)*0.50)
                     + pfAdj + homeAdv + hBullAdj - aBullAdj
                     - hInjAdj + aInjAdj + hRDadj;
    var awayOwnScore = (aOffScore*0.50 + (1-hPitchScore)*0.50)
                     - pfAdj + aBullAdj - hBullAdj
                     - aInjAdj + hInjAdj - hRDadj;
    ...
    // ── MODELO FINAL — ratio dinámico + ELO de Natstat ────────
    ...
    // Natstat MLB ELO as tiebreaker (5% weight max)
    var hMLBelo=getNatstatELO('MLB',homeTeam);
    var aMLBelo=getNatstatELO('MLB',awayTeam);
    var eloAdj=0;
    if(hMLBelo&&hMLBelo.elo&&aMLBelo&&aMLBelo.elo){
      var mlbEloDiff=hMLBelo.elo-aMLBelo.elo;
      var eloProb=clamp(1/(1+Math.pow(10,-mlbEloDiff/400)),0.1,0.9);
      eloAdj=(eloProb-0.5)*0.05; // max ±2.5% adjustment
    }
    var modelHome=clamp(ownProbHome*_r.own + mktHome*_r.mkt + eloAdj, 0.05, 0.95);
    ...
    // betH = model favors home AND edge is positive for home
    // If edge is negative, we favor away (market has more info)
    var betH=modelHome>=modelAway; // model favorite
    var betEdge=betH?edgeH:edgeA;  // edge for model favorite
    var betTeam=betH?homeTeam:awayTeam;
    var betModel=betH?modelHome:modelAway;
    var betMkt_=betH?mktHome:mktAway;
    var betOdd=betH?bestOddH:bestOddA;
    var evVal=oddsEv?(betModel*(betOdd-1))-(1-betModel):null;

    // ── Betting splits + Sharp money ──────────────────────────
    var splits=findBettingSplits(homeTeam,awayTeam);
    var sharpSignal=null;
    var splitsHTML='';
    if(splits){
      var hBets=splits.ml_home_pct_bets||0,aBets=splits.ml_away_pct_bets||0;
      var hMon=splits.ml_home_pct_money||0,aMon=splits.ml_away_pct_money||0;
      if(splits.sharp_signal) sharpSignal=splits.sharp_signal;
      else if(hBets&&hMon){
        var diff=hMon-hBets;
        if(diff>=20) sharpSignal='⚡ Sharp money → '+homeTeam.split(' ').pop()+' (+'+diff+'%)';
        else if(diff<=-20) sharpSignal='⚠️ Sharp money → '+awayTeam.split(' ').pop()+' ('+Math.abs(diff)+'%)';
      }
      // Show splits panel if we have either public bets or implied probability
      var hImpl=splits.home_implied_prob||0, aImpl=splits.away_implied_prob||0;
      if(hBets||aBets||hImpl){
        var hasRealSplits=!!(hBets||aBets);
        splitsHTML=
          '<div style="background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:10px;margin-top:4px">'+
            '<div style="font-family:DM Mono,monospace;font-size:0.62rem;color:var(--muted);text-transform:uppercase;letter-spacing:0.08em;margin-bottom:6px">'+
              (hasRealSplits?'📊 Splits del público':'📊 Probabilidad implícita ('+(splits.books_count||'?')+' casas)')+
            '</div>'+
            '<div style="display:grid;grid-template-columns:1fr auto 1fr;gap:4px;font-size:0.75rem;text-align:center;margin-bottom:4px">'+
              '<div><div style="font-weight:700;color:var(--text)">'+homeAbbr+'</div>'+
                (hasRealSplits
                  ?'<div style="color:'+(hBets>65?'#ef4444':'var(--muted)')+'">'+hBets+'% bets</div>'+
                    '<div style="color:'+(hMon>65?'#10b981':'var(--muted)')+'">'+hMon+'% $$$</div>'
                  :'<div style="color:'+(hImpl>60?'#10b981':'var(--muted)')+'">'+hImpl+'% impl</div>'
                )+'</div>'+
              '<div style="color:var(--dim);align-self:center">vs</div>'+
              '<div><div style="font-weight:700;color:var(--text)">'+awayAbbr+'</div>'+
                (hasRealSplits
                  ?'<div style="color:'+(aBets>65?'#ef4444':'var(--muted)')+'">'+aBets+'% bets</div>'+
                    '<div style="color:'+(aMon>65?'#10b981':'var(--muted)')+'">'+aMon+'% $$$</div>'
                  :'<div style="color:'+(aImpl>60?'#10b981':'var(--muted)')+'">'+aImpl+'% impl</div>'
                )+'</div>'+
            '</div>'+
            (splits.total_line?'<div style="font-size:0.68rem;color:var(--muted);text-align:center">O/U '+splits.total_line+' · Over: '+(splits.over_pct_bets||'?')+'%</div>':'')+
            (sharpSignal?'<div style="font-size:0.72rem;font-weight:700;color:#f59e0b;text-align:center;margin-top:4px">'+sharpSignal+'</div>':'')+
            (!hasRealSplits?'<div style="font-size:0.62rem;color:var(--dim);text-align:center;margin-top:3px">Sin datos de splits públicos disponibles</div>':'')+
          '</div>';
      }
    }
    ...
    // ── EV calculation ─────────────────────────────────────────
    // EV = (prob_modelo × ganancia) - (prob_fallo × stake)
    var evHome=oddsEv?(modelHome*(bestOddH-1))-(1-modelHome):null;
    var evAway=oddsEv?(modelAway*(bestOddA-1))-(1-modelAway):null;
    var bestEV=null,bestEVteam='',bestEVodd=1,bestEVprob=0;
    if(evHome!==null&&evAway!==null){
      if(evHome>=evAway){ bestEV=evHome;bestEVteam=homeTeam;bestEVodd=bestOddH;bestEVprob=modelHome; }
      else { bestEV=evAway;bestEVteam=awayTeam;bestEVodd=bestOddA;bestEVprob=modelAway; }
    }

    // ── Cuota mínima recomendada ───────────────────────────────
    // Cuota mínima = 1 / prob_modelo (break-even sin vig)
    // Con vig del 5%: cuota_min = 1.05 / prob_modelo
    var minOddHome=modelHome>0?Math.round((1.05/modelHome)*100)/100:null;
    var minOddAway=modelAway>0?Math.round((1.05/modelAway)*100)/100:null;

    // ── Señales de valor ───────────────────────────────────────
    var valueSigns=[];
    // Bullpen fatigue
    if(homeBullpen&&homeBullpen.fatigue>0.5)
      valueSigns.push({icon:'😤',text:'Bullpen local agotado',detail:homeBullpen.recentIP.toFixed(1)+'IP/3d',side:'home',positive:false});
    if(awayBullpen&&awayBullpen.fatigue>0.5)
      valueSigns.push({icon:'😤',text:'Bullpen visita agotado',detail:awayBullpen.recentIP.toFixed(1)+'IP/3d',side:'away',positive:false});
    // Fresh bullpen
    if(homeBullpen&&homeBullpen.fatigue<0.2&&homeBullpen.avgERA&&parseFloat(homeBullpen.avgERA)<3.8)
      valueSigns.push({icon:'💪',text:'Bullpen local fresco y élite',detail:'ERA '+homeBullpen.avgERA,side:'home',positive:true});
    if(awayBullpen&&awayBullpen.fatigue<0.2&&awayBullpen.avgERA&&parseFloat(awayBullpen.avgERA)<3.8)
      valueSigns.push({icon:'💪',text:'Bullpen visita fresco y élite',detail:'ERA '+awayBullpen.avgERA,side:'away',positive:true});
    // Pitcher splits home vs away
    if(hpSplits&&hpSplits.home&&hpSplits.away){
      var hEraH=parseFloat(hpSplits.home.era),hEraA=parseFloat(hpSplits.away.era);
      if(!isNaN(hEraH)&&!isNaN(hEraA)&&Math.abs(hEraH-hEraA)>=1.5){
        if(isHome&&hEraH<hEraA) valueSigns.push({icon:'🏠',text:hp.name.split(' ').pop()+' mejor en casa',detail:'ERA casa '+hEraH.toFixed(2)+' vs visita '+hEraA.toFixed(2),side:'home',positive:true});
        if(!isHome&&hEraA<hEraH) valueSigns.push({icon:'✈️',text:hp.name.split(' ').pop()+' mejor de visitante',detail:'ERA vis '+hEraA.toFixed(2)+' vs casa '+hEraH.toFixed(2),side:'home',positive:true});
        if(isHome&&hEraH>hEraA) valueSigns.push({icon:'⚠️',text:hp.name.split(' ').pop()+' peor en casa',detail:'ERA casa '+hEraH.toFixed(2)+' vs visita '+hEraA.toFixed(2),side:'home',positive:false});
      }
    }
    if(apSplits&&apSplits.home&&apSplits.away){
      var aEraH=parseFloat(apSplits.home.era),aEraA=parseFloat(apSplits.away.era);
      if(!isNaN(aEraH)&&!isNaN(aEraA)&&Math.abs(aEraH-aEraA)>=1.5){
        if(!isHome&&aEraH<aEraA) valueSigns.push({icon:'🏠',text:ap.name.split(' ').pop()+' mejor en casa',detail:'ERA casa '+aEraH.toFixed(2)+' vs visita '+aEraA.toFixed(2),side:'away',positive:true});
        if(isHome&&aEraA<aEraH) valueSigns.push({icon:'✈️',text:ap.name.split(' ').pop()+' mejor de visitante',detail:'ERA vis '+aEraA.toFixed(2)+' vs casa '+aEraH.toFixed(2),side:'away',positive:true});
      }
    }
    // IL impact
    if(homeImpact.length>=2) valueSigns.push({icon:'🤕',text:homeAbbr+' '+homeImpact.length+' titulares en IL',detail:homeImpact.map(function(i){return i.pos;}).join(', '),side:'home',positive:false});
    if(awayImpact.length>=2) valueSigns.push({icon:'🤕',text:awayAbbr+' '+awayImpact.length+' titulares en IL',detail:awayImpact.map(function(i){return i.pos;}).join(', '),side:'away',positive:false});
    // Run differential
    var hStd=findStandings(homeTeam),aStd=findStandings(awayTeam);
    if(hStd&&aStd&&hStd.run_diff!=null&&aStd.run_diff!=null){
      var rdDiff=hStd.run_diff-aStd.run_diff;
      if(rdDiff>30) valueSigns.push({icon:'📈',text:'Local supera en run diff',detail:'+'+(rdDiff.toFixed(0))+' runs',side:'home',positive:true});
      if(rdDiff<-30) valueSigns.push({icon:'📈',text:'Visita supera en run diff',detail:rdDiff.toFixed(0)+' runs',side:'away',positive:true});
    }
    // Lineup not confirmed
    if(lineup&&!lineup.homeConfirmed&&!lineup.awayConfirmed)
      valueSigns.push({icon:'⏳',text:'Lineups no confirmados',detail:'Modelo usa proyección',side:'both',positive:false});
    // Model disagrees with market significantly
    if(oddsEv&&absEdge>=0.08&&!modelAgreesMkt)
      valueSigns.push({icon:'⚡',text:'Modelo difiere del mercado',detail:modelFav+' '+modelFavPct+'% vs Mkt '+mktFavPct+'%',side:'model',positive:null});

    var recC,recL;
    // betEdge: positive = model sees value, negative = market disagrees with model
    var edgePositive=betEdge>0;

    if(!pitchOK){
      recC='rec-avoid';
      recL='⏳ Pitchers TBD — predicción pendiente';
    } else if(oddsEv&&absEdge>=0.05&&edgePositive&&goodOdds&&!windBad){
      // Strong positive edge — recommend bet
      recC='rec-good';
      recL='🟢 PICK: '+betTeam+' · Modelo '+Math.round(betModel*100)+'% vs Mkt '+Math.round(betMkt_*100)+'% · '+modelConf+alertStr;
    } else if(oddsEv&&absEdge>=0.05&&!edgePositive&&goodOdds){
      // Strong edge but AGAINST model favorite — recommend opposite
      var oppTeam=betH?awayTeam:homeTeam;
      var oppModel=betH?modelAway:modelHome;
      var oppMkt=betH?mktAway:mktHome;
      recC='rec-tight';
      recL='🟡 Mercado prefiere: '+oppTeam+' · Mkt '+Math.round(oppMkt*100)+'% vs Modelo '+Math.round(oppModel*100)+'% — mercado tiene edge'+alertStr;
    } else if(oddsEv&&absEdge>=0.02&&edgePositive){
      recC='rec-tight';
      recL='🟡 LEVE: '+betTeam+' · Modelo '+Math.round(betModel*100)+'% · '+modelConf+alertStr;
    } else if(!oddsEv&&modelFavPct>=62){
      recC='rec-tight';
      recL='🟡 '+modelFav+' (Modelo '+modelFavPct+'% · sin odds)'+alertStr;
    } else if(oddsEv&&modelFavPct>=62&&edgePositive){
      recC=modelAgreesMkt?'rec-good':'rec-tight';
      recL=(modelAgreesMkt?'🟢':'🟡')+' '+modelFav+' (Modelo '+modelFavPct+'%'+' · Mkt '+mktFavPct+'%'+')'+(modelAgreesMkt?'':' ⚡ Difiere del mkt')+alertStr;
    } else if(modelFavPct>=54&&edgePositive){
      recC='rec-tight';
      recL='🟡 '+modelFav+' leve · Modelo '+modelFavPct+'%'+(oddsEv?' · Mkt '+mktFavPct+'%':'')+alertStr;
    } else {
      recC='rec-avoid';
      recL='🔴 Parejo · '+modelFav+' '+modelFavPct+'% — sin edge claro'+alertStr;
    }
    ...
    var modelBreakdown=
      ...
      (function(){
        var hs=findStandings(homeTeam),as_=findStandings(awayTeam);
        var hf=findFirst5(homeTeam),af=findFirst5(awayTeam);
        var parts=[];
        if(hs&&hs.run_diff!=null) parts.push('🏠 RunDiff: '+(hs.run_diff>0?'+':'')+hs.run_diff);
        if(as_&&as_.run_diff!=null) parts.push('✈️ RunDiff: '+(as_.run_diff>0?'+':'')+as_.run_diff);
        if(hf) parts.push('F5 casa: '+hf.avg_scored_f5+'R/G');
        if(af) parts.push('F5 visita: '+af.avg_scored_f5+'R/G');
        return parts.length?'<div style="font-size:0.65rem;color:#555;margin-top:2px;text-align:center">'+parts.join(' · ')+'</div>':'';
      })()+
    ...
    div.innerHTML=
      '<div style="font-family:DM Mono,monospace;font-size:0.68rem;color:var(--muted);display:flex;justify-content:space-between;align-items:center">'+
        ...
        '<span style="display:flex;gap:6px">'+
          (lineup&&lineup.homeConfirmed
            ...
            :lineup&&lineup.isProbable&&lineup.homeProbable&&lineup.homeProbable.length
              ?'<span style="color:#3b82f6;font-size:0.63rem">🔵 Probable local</span>'
          ...
          (lineup&&lineup.awayConfirmed
            ...
            :lineup&&lineup.isProbable&&lineup.awayProbable&&lineup.awayProbable.length
              ?'<span style="color:#3b82f6;font-size:0.63rem">🔵 Probable visita</span>'
      ...
      // ── F5 Panel ─────────────────────────────────────────────
      (function(){
        var hpL5=findPitcherL5(hp.id);
        var apL5=findPitcherL5(ap.id);
        var f5Odds=findF5Odds(homeTeam,awayTeam);
        var f5=calcF5Prediction(homeTeam,awayTeam,hp,ap,hpL5,apL5,f5Odds);
        var f5Fav=f5.f5ModelHome>=0.5?homeTeam:awayTeam;
        var f5FavPct=Math.round(Math.max(f5.f5ModelHome,f5.f5ModelAway)*100);
        var f5Edge=f5.f5ModelHome>=0.5?(f5.f5ModelHome-f5.f5MktHome):(f5.f5ModelAway-(1-f5.f5MktHome));
        var f5BestOdd=f5.f5ModelHome>=0.5?(f5Odds&&f5Odds.f5_home_odds||0):(f5Odds&&f5Odds.f5_away_odds||0);
        var f5EV=f5BestOdd>1?(f5FavPct/100*(f5BestOdd-1)-(1-f5FavPct/100)):null;
        var f5RecC=f5FavPct>=62&&f5Edge>0.04?'rec-good':f5FavPct>=55?'rec-tight':'rec-avoid';
        var f5RecL=f5FavPct>=62&&f5Edge>0.04
          ?'🟢 F5: '+f5Fav+' '+f5FavPct+'% · Edge +'+(f5Edge*100).toFixed(0)+'%'
          :f5FavPct>=55?'🟡 F5 leve: '+f5Fav+' '+f5FavPct+'%'
          :'🔴 F5 parejo · '+f5Fav+' '+f5FavPct+'%';
        return '<div style="background:var(--surface2);border:1px solid #7f5af044;border-radius:10px;padding:12px;margin-top:4px">'+
          '<div style="font-family:DM Mono,monospace;font-size:0.62rem;color:#7f5af0;text-transform:uppercase;letter-spacing:0.08em;margin-bottom:8px">⚾ First 5 Innings</div>'+
          '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px">'+
            '<div class="mlb-team-col">'+
              '<div class="mlb-row-section">🏠 '+homeAbbr+'</div>'+
              '<div class="mlb-row"><span>ERA (L5)</span><span style="color:'+(f5.hEraL5&&f5.hEraL5<3.5?'#10b981':f5.hEraL5&&f5.hEraL5<4.5?'#f59e0b':'#ef4444')+'">'+(f5.hEraL5||hp.era||'—')+'</span></div>'+
              '<div class="mlb-row"><span>IP/salida</span><span>'+(f5.hAvgIp||'—')+'</span></div>'+
              (hpL5&&hpL5.game_log?'<div class="mlb-row"><span>L5 starts</span><span style="font-size:0.8rem">'+hpL5.game_log.map(function(g){return g.er===0?'✅':g.er<=2?'🟡':'❌';}).join('')+'</span></div>':'')+
            '</div>'+
            '<div class="mlb-team-col">'+
              '<div class="mlb-row-section">✈️ '+awayAbbr+'</div>'+
              '<div class="mlb-row"><span>ERA (L5)</span><span style="color:'+(f5.aEraL5&&f5.aEraL5<3.5?'#10b981':f5.aEraL5&&f5.aEraL5<4.5?'#f59e0b':'#ef4444')+'">'+(f5.aEraL5||ap.era||'—')+'</span></div>'+
              '<div class="mlb-row"><span>IP/salida</span><span>'+(f5.aAvgIp||'—')+'</span></div>'+
              (apL5&&apL5.game_log?'<div class="mlb-row"><span>L5 starts</span><span style="font-size:0.8rem">'+apL5.game_log.map(function(g){return g.er===0?'✅':g.er<=2?'🟡':'❌';}).join('')+'</span></div>':'')+
            '</div>'+
          '</div>'+
          '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;margin-bottom:8px">'+
            '<div class="mlb-odd-box"><div class="mlb-odd-label">🧠 F5 Modelo</div><div class="mlb-odd-value" style="color:'+(f5.f5ModelHome>0.5?'#10b981':'#ef4444')+'">'+Math.round(f5.f5ModelHome*100)+'%</div></div>'+
            '<div class="mlb-odd-box"><div class="mlb-odd-label">🏪 F5 Mercado</div><div class="mlb-odd-value" style="color:#aaa">'+Math.round(f5.f5MktHome*100)+'%</div></div>'+
            '<div class="mlb-odd-box"><div class="mlb-odd-label">⚡ F5 Edge</div><div class="mlb-odd-value" style="color:'+(f5Edge>0.04?'#10b981':f5Edge>0?'#f59e0b':'#ef4444')+'">'+(f5Edge>=0?'+':'')+Math.round(f5Edge*100)+'%</div></div>'+
          '</div>'+
          (f5EV!==null?'<div style="font-size:0.72rem;color:var(--muted);text-align:center;margin-bottom:6px">EV F5: '+(f5EV>0?'+':'')+( f5EV*100).toFixed(1)+' por $100</div>':'')+
          '<div class="mlb-rec-badge '+f5RecC+'">'+f5RecL+'</div>'+
        '</div>';
      })()+
  ...
  window._loadLeague = async function(leagueKey){
    ...
    pitcherSplitsCache={}; bullpenCache={}; injuryCache={}; natstatJSON=null;
    // Don't reset oddsCache — preserve 30min stability
    propsDataStore={};
