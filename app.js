...
document.addEventListener('DOMContentLoaded', function () {
  ...
  async function buildNPBGameFromData(game,oddsData){
    ...
    div.innerHTML=
      '<div style="font-size:0.72rem;color:#e91e7a;display:flex;justify-content:space-between;align-items:center">'+
        '<span>🇯🇵 NPB</span>'+
  ...
  async function loadNPBGamesPanel(){
    ...
    try{
      ...
      var todayGames=(npbJSON&&npbJSON.npb_games_today)||
                     buildNPBGamesFromProbable(npbJSON&&npbJSON.npb_probable_pitchers||{});
      var tomorrowGames=(npbJSON&&npbJSON.npb_games_tomorrow)||
                        buildNPBGamesFromProbable(npbJSON&&npbJSON.npb_probable_tomorrow||{});
      ...
      var tomorrowDateObj=new Date(now); tomorrowDateObj.setDate(tomorrowDateObj.getDate()+1);
      var tomorrowDate=tomorrowDateObj.toLocaleDateString('es-PA',{weekday:'short',month:'short',day:'numeric'});
      ...
      hdr.innerHTML=
        '<div style="font-size:0.72rem;color:#e91e7a;text-transform:uppercase;letter-spacing:0.08em">'+
          '🇯🇵 NPB Béisbol Japonés · actualizado: '+genDate+
        '</div>'+
        '<div style="display:flex;gap:8px">'+
          '<button id="npbBtnHoy" onclick="window._npbShowDay(\'hoy\')" style="flex:1;padding:9px 6px;border-radius:8px;border:1px solid #e91e7a;background:#e91e7a22;color:#e91e7a;font-size:0.78rem;font-weight:700;cursor:pointer">'+
            '📅 Hoy · '+todayDate+(todayGames.length?' ('+todayGames.length+')':" · sin datos")+
          '</button>'+
          '<button id="npbBtnMan" onclick="window._npbShowDay(\'manana\')" style="flex:1;padding:9px 6px;border-radius:8px;border:1px solid #555;background:transparent;color:#888;font-size:0.78rem;font-weight:600;cursor:pointer">'+
      ...
      window._npbShowDay=async function(day){
        ...
        if(!games.length){
          container.innerHTML='<div style="text-align:center;padding:24px;color:#555;font-size:0.85rem">'+
            (day==='hoy'
              ?'Sin partidos hoy · Revisa la pestaña Mañana 👆'
              :'Sin datos para mañana aún · npb.jp los publica ~6pm hora Japón (4am Panamá)')+
          '</div>';
          return;
        }
        container.innerHTML='<div style="text-align:center;color:#555;padding:8px;font-size:0.75rem">⏳ Cargando '+games.length+' partidos...</div>';
        container.innerHTML='';
