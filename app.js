  ...
  function getTeamBattersForProps(teamName, side, lineup, mlbHit, teamId, teamGP){
    ...
    if(lineup && lineup.length >= 4){
      return lineup.slice(0, 9).map(function(b){
        ...
        var pitcherRivalHand = null; // hand context not available at lineup level
        // Platoon split vs pitcher hand
        var xbaVsPitcher = null;
        if(scM && pitcherRivalHand){
          xbaVsPitcher = pitcherRivalHand==='L' ? scM.xBA_vsL : scM.xBA_vsR;
        }
        // Forma reciente (últimos 14d)
        var br = mlbStatsJSON ? mlbStatsJSON.batting_recent : null;
        var brM = br ? br.find(function(r){
          return r.Name && r.Name.toLowerCase().split(' ').pop() === b.name.toLowerCase().split(' ').pop();
        }) : null;

        return {
          name: b.name,
          avg:  fgM && fgM.AVG ? fgM.AVG : (b.avg !== '—' ? parseFloat(b.avg) : 0.250),
          avgRecent: brM && brM.AVG ? brM.AVG : null,
          xba:  xbaVsPitcher || (scM ? scM.xBA : null),
          ops:  b.ops !== '—' ? b.ops : (fgM && fgM.OPS ? fgM.OPS : null),
          opsRecent: brM && brM.OPS ? brM.OPS : null,
          wrc: fgM ? fgM['wRC+'] : null,
          wrcRecent: brM ? brM['wRC+'] : null,
          iso: fgM ? fgM.ISO : null,
          babip: fgM ? fgM.BABIP : null,
          exitVel: scM ? scM.avg_exit_vel : null,
          hardHit: scM ? scM.hard_hit_pct : null,
          barrel: scM ? scM.barrel_pct : null,
          batSide: scM ? scM.bat_side : null,
