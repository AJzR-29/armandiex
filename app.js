...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function findBettingSplits(homeTeam, awayTeam){
    ...
    return mlbStatsJSON.betting_splits.find(function(g){
      return (g.home&&g.home.toLowerCase().includes(hn)&&
              g.away&&g.away.toLowerCase().includes(an))||
             (g.home&&g.home.toLowerCase().includes(an)&&
              g.away&&g.away.toLowerCase().includes(hn));
    })||null;
  }
  ...
  async function buildNPBGameFromData(game,oddsData){
    ...
    // Sharp signal validation
    if(sharpSignal){
      var sharpTeam=sharpSignal.indexOf(betTeam)>=0?betTeam:null;
      if(sharpTeam) recL+=' · ⚡ Sharp confirma';
  ...
  async function buildGameCard(ev,oddsData,mlbSched,mlbHit,isMLB,leagueKey){
    ...
    var splitsHTML='';
    if(splits){
      var mlBetsPct=betH?(splits.ml_home_pct_bets||0):(splits.ml_away_pct_bets||0);
      var mlMoneyPct=betH?(splits.ml_home_pct_money||0):(splits.ml_away_pct_money||0);
      if(splits.sharp_signal){
        sharpSignal=splits.sharp_signal;
      } else if(mlBetsPct&&mlMoneyPct){
        var diff=mlMoneyPct-mlBetsPct;
        if(diff>=20) sharpSignal='⚡ Sharp money → '+betTeam+' (+'+diff+'%)';
        else if(diff<=-20) sharpSignal='⚠️ Sharp money → '+(betH?awayTeam:homeTeam)+' ('+Math.abs(diff)+'%)';
      }
      ...
      if(hBets||aBets){
        splitsHTML=
          '<div style="background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:10px;margin-top:4px">'+
            '<div style="font-family:DM Mono,monospace;font-size:0.62rem;color:var(--muted);text-transform:uppercase;letter-spacing:0.08em;margin-bottom:6px">📊 Splits del público</div>'+
            '<div style="display:grid;grid-template-columns:1fr auto 1fr;gap:4px;font-size:0.75rem;text-align:center;margin-bottom:4px">'+
              '<div><div style="font-weight:700">'+homeAbbr+'</div>'+
                '<div style="color:'+(hBets>65?'#ef4444':'var(--muted)')+'">'+hBets+'% bets</div>'+
                '<div style="color:'+(hMon>65?'#10b981':'var(--muted)')+'">'+hMon+'% $$$</div></div>'+
              '<div style="color:var(--dim);align-self:center">vs</div>'+
              '<div><div style="font-weight:700">'+awayAbbr+'</div>'+
                '<div style="color:'+(aBets>65?'#ef4444':'var(--muted)')+'">'+aBets+'% bets</div>'+
                '<div style="color:'+(aMon>65?'#10b981':'var(--muted)')+'">'+aMon+'% $$$</div></div>'+
            '</div>'+
    ...
    if(splits){
      var mlBetsPct = betH ? (splits.ml_home_pct_bets||0) : (splits.ml_away_pct_bets||0);
      var mlMoneyPct= betH ? (splits.ml_home_pct_money||0): (splits.ml_away_pct_money||0);
      var oppBetsPct= betH ? (splits.ml_away_pct_bets||0) : (splits.ml_home_pct_bets||0);
      var oppMoneyPct=betH ? (splits.ml_away_pct_money||0): (splits.ml_home_pct_money||0);

      // Sharp signal: big divergence between % bets and % money
      if(splits.sharp_signal){
        sharpSignal=splits.sharp_signal;
      } else if(mlBetsPct&&mlMoneyPct){
        var diff=mlMoneyPct-mlBetsPct;
        if(diff>=20) sharpSignal='⚡ Sharp money → '+betTeam+' (+'+diff+'% dinero vs apuestas)';
        else if(diff<=-20) sharpSignal='⚠️ Sharp money → '+(betH?awayTeam:homeTeam)+' ('+diff+'% dinero vs apuestas)';
      }

      // Build splits display
      if(splits.ml_home_pct_bets||splits.ml_away_pct_bets){
        var hBets=splits.ml_home_pct_bets||0, aBets=splits.ml_away_pct_bets||0;
        var hMon=splits.ml_home_pct_money||0, aMon=splits.ml_away_pct_money||0;
        var overBets=splits.over_pct_bets||0, totalLine=splits.total_line;
        splitsHTML=
          '<div style="background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:10px;margin-top:4px">'+
            '<div style="font-family:DM Mono,monospace;font-size:0.62rem;color:var(--muted);text-transform:uppercase;letter-spacing:0.08em;margin-bottom:6px">📊 Splits del público</div>'+
            '<div style="display:grid;grid-template-columns:1fr auto 1fr;gap:4px;font-size:0.75rem;text-align:center;margin-bottom:6px">'+
              '<div>'+
                '<div style="font-weight:700;color:var(--text)">'+homeAbbr+'</div>'+
                '<div style="color:'+(hBets>60?'#ef4444':'var(--muted)')+'">'+hBets+'% bets</div>'+
                '<div style="color:'+(hMon>60?'#10b981':'var(--muted)')+'">'+hMon+'% $$$</div>'+
              '</div>'+
              '<div style="color:var(--dim);align-self:center">vs</div>'+
              '<div>'+
                '<div style="font-weight:700;color:var(--text)">'+awayAbbr+'</div>'+
                '<div style="color:'+(aBets>60?'#ef4444':'var(--muted)')+'">'+aBets+'% bets</div>'+
                '<div style="color:'+(aMon>60?'#10b981':'var(--muted)')+'">'+aMon+'% $$$</div>'+
              '</div>'+
            '</div>'+
            (totalLine?'<div style="font-size:0.68rem;color:var(--muted);text-align:center">O/U: '+totalLine+' · Over: '+overBets+'%</div>':'')+
            (sharpSignal?'<div style="font-size:0.72rem;font-weight:700;color:#f59e0b;text-align:center;margin-top:4px">'+sharpSignal+'</div>':'')+
          '</div>';
      }
    }
