...
document.addEventListener('DOMContentLoaded', function () {
  ...
  async function buildNPBGameFromData(game, oddsData){
    // Support both {home,away} and {homeTeam,awayTeam} formats
  ...
  async function loadNPBGamesPanel(){
    ...
    try{
      ...
      // Use npb_games arrays directly (populated by Python with hardcoded fallback)
      var todayGames=(npbJSON&&npbJSON.npb_games_today)||[];
      var tomorrowGames=(npbJSON&&npbJSON.npb_games_tomorrow)||[];

      // Fallback to probable_pitchers format if games arrays not present
      ...
      // Last resort: fetch from npb.jp directly
      if(!todayGames.length&&!tomorrowGames.length){
        var schedMap=await fetchNPBSchedule();
        todayGames=buildNPBGamesFromProbable(schedMap);
      }

      mlbPanel.innerHTML='';

      // ── Header con toggle Hoy / Mañana ────────────────────────
      var genDate=npbJSON&&npbJSON.generated_at||'—';
      var hdr=document.createElement('div');
      hdr.style.cssText='display:flex;flex-direction:column;gap:8px;padding:4px 0';
      hdr.innerHTML=
        '<div style="font-size:0.75rem;color:#e91e7a;text-transform:uppercase;letter-spacing:0.08em">'+
          '🇯🇵 NPB Béisbol Japonés · actualizado: '+genDate+
        '</div>'+
        '<div style="display:flex;gap:8px">'+
          '<button id="npbBtnHoy" onclick="window._npbShowDay(\'hoy\')" style="flex:1;padding:8px;border-radius:8px;border:1px solid #e91e7a;background:#e91e7a22;color:#e91e7a;font-size:0.82rem;font-weight:700;cursor:pointer">'+
            '📅 Hoy'+(todayGames.length?' ('+todayGames.length+')':' (sin datos)')+
          '</button>'+
          '<button id="npbBtnMan" onclick="window._npbShowDay(\'manana\')" style="flex:1;padding:8px;border-radius:8px;border:1px solid #555;background:transparent;color:#888;font-size:0.82rem;font-weight:600;cursor:pointer">'+
            '📅 Mañana'+(tomorrowGames.length?' ('+tomorrowGames.length+')':' (sin datos)')+
          '</button>'+
        '</div>';
      ...
      gamesContainer.id='npbGamesContainer';
      gamesContainer.style.cssText='display:flex;flex-direction:column;gap:14px';
      mlbPanel.appendChild(gamesContainer);

      // Store games for toggle
      ...
      window._npbShowDay=async function(day){
        ...
        var container=document.getElementById('npbGamesContainer');
        if(!container) return;
        if(!games.length){
          container.innerHTML='<div style="text-align:center;padding:24px;color:#555;font-size:0.85rem">'+
            (day==='hoy'?'Sin partidos hoy. Revisa la pestaña Mañana 👆':'Sin datos para mañana aún — npb.jp los publica ~6pm hora Japón (4am Panamá)')+
          '</div>';
          return;
        }
        container.innerHTML='<div style="text-align:center;color:#555;padding:12px;font-size:0.8rem">⏳ Cargando tarjetas...</div>';
        container.innerHTML='';
        for(var i=0;i<games.length;i++){
          container.appendChild(await buildNPBGameFromData(games[i],window._npbOddsData));
        }
      };

      // Show today by default, or tomorrow if no games today
  ...
  sportSelect.addEventListener('change',function(){
    ...
    pitcherSplitsCache={}; bullpenCache={}; npbScheduleCache=null; npbStatsJSON=null;
