...
try:
    ...
    # SportsBettingDime public betting trends
    ...
    if sbd_r.ok:
        ...
        game_containers = soup.find_all('div', class_=re.compile(r'game|match|event', re.I))
        if not game_containers:
            game_containers = soup.find_all('tr')

        print(f"   Containers found: {len(game_containers)}")

        # Try to find JSON data embedded in page (many sites do this)
        scripts = soup.find_all('script', type='application/json')
        for sc in scripts:
            try:
                data = json.loads(sc.string)
                print(f"   JSON script found: {str(data)[:200]}")
            except: pass
        ...
        if next_data:
            try:
                ...
                # Navigate to games data
                props = nd.get('props',{}).get('pageProps',{})
                print(f"   pageProps keys: {list(props.keys())[:10]}")
                games_data = props.get('games') or props.get('data') or props.get('events')
                if games_data:
                    print(f"   Games in __NEXT_DATA__: {len(games_data)}")
                    print(f"   First game sample: {str(games_data[0])[:300]}")
            except Exception as e:
                print(f"   __NEXT_DATA__ parse error: {e}")

        # Look for window.__STATE__ or similar inline data
        all_scripts = soup.find_all('script')
        for sc in all_scripts:
            if sc.string and ('bets' in sc.string.lower() or 'consensus' in sc.string.lower() or 'percent' in sc.string.lower()):
                print(f"   Found relevant script: {sc.string[:300]}")
                break
