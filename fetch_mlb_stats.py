...
# ════════════════════════════════════════════════════════
# BETTING SPLITS — ActionNetwork API (gratis, sin key)
# ════════════════════════════════════════════════════════
print("→ Betting splits (ActionNetwork)...")
betting_splits = []
...
try:
    ...
    if an_r.ok:
        ...
        for g in an_games:
            ...
            home_team = next((t for t in teams if t.get('id') == home_id), teams[1] if len(teams)>1 else {})
            away_team = next((t for t in teams if t.get('id') != home_id), teams[0])
            ...
            ml_con  = consensus.get('moneyline') or {}
            rl_con  = consensus.get('spread') or {}
            tot_con = consensus.get('total') or {}

            # Get O/U line from odds
            total_line = None
            for odds in (g.get('odds') or []):
                if odds.get('period') == 0 and odds.get('total'):
                    total_line = safe(odds.get('total'))
                    break
            ...
            # Auto-detect sharp money signal
            h_bets = split['ml_home_pct_bets'] or 0
            h_money = split['ml_home_pct_money'] or 0
            if h_bets and h_money:
                diff = h_money - h_bets
                if diff >= 20:
                    split['sharp_signal'] = f"⚡ Sharp money → {home_name.split()[-1]} (+{diff}%)"
                elif diff <= -20:
                    split['sharp_signal'] = f"⚠️ Sharp money → {away_name.split()[-1]} ({diff}%)"

            betting_splits.append(split)

        print(f"   betting_splits: {len(betting_splits)} games")
    else:
        print(f"   ActionNetwork failed: {an_r.status_code} {an_r.text[:100]}")
