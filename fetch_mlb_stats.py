...
# ════════════════════════════════════════════════════════
# F5 PITCHER STATS — Last 5 starts ERA via MLB-StatsAPI
# ════════════════════════════════════════════════════════
print("→ F5 pitcher recent form (last 5 starts)...")
pitcher_l5 = {}  # {pitcher_id: {era_l5, ip_l5, k_l5, bb_l5, avg_innings}}
...
try:
    ...
    pitcher_ids = set()
    for de in (sched_data.get('dates') or []):
        for game in (de.get('games') or []):
    ...
    print(f"   Probable pitchers today: {len(pitcher_ids)}")
    
    for pid in pitcher_ids:
        try:
            time.sleep(0.2)
            log_url = f"{MLBAPI}/people/{pid}/stats?stats=gameLog&group=pitching&season={SEASON}&sportId=1"
            r = requests.get(log_url, timeout=10)
            if not r.ok: continue
            data = r.json()
            splits = (data.get('stats') or [{}])[0].get('splits') or []
            
            # Filter to starts only (gamesStarted=1), last 5
            starts = [s for s in splits if int((s.get('stat') or {}).get('gamesStarted',0)) >= 1]
            last5 = starts[-5:] if len(starts) >= 5 else starts
            
            if not last5: continue
            
            total_er = sum(int((s.get('stat') or {}).get('earnedRuns',0)) for s in last5)
            total_ip = sum(float((s.get('stat') or {}).get('inningsPitched',0)) for s in last5)
            total_k  = sum(int((s.get('stat') or {}).get('strikeOuts',0)) for s in last5)
            total_bb = sum(int((s.get('stat') or {}).get('baseOnBalls',0)) for s in last5)
            
            era_l5 = round((total_er / total_ip * 9), 2) if total_ip > 0 else None
            avg_ip = round(total_ip / len(last5), 1) if last5 else None
            k9_l5  = round(total_k / total_ip * 9, 2) if total_ip > 0 else None
            ...
            pitcher_l5[pid] = {
                'era_l5':    era_l5,
                'ip_l5':     round(total_ip,1),
                'avg_ip':    avg_ip,
                'k9_l5':     k9_l5,
                'starts':    len(last5),
                'game_log':  game_scores,
            }
        except Exception as e:
            pass
    
    print(f"   pitcher_l5: {len(pitcher_l5)} pitchers")

except Exception as e:
    print(f"   pitcher_l5 error: {e}")
    pitcher_l5 = {}

# ════════════════════════════════════════════════════════
# F5 ODDS — The Odds API first_five_innings market
# ════════════════════════════════════════════════════════
print("→ F5 odds (The Odds API)...")
f5_odds = []
try:
    ODDS_KEY = "27efeb9368c8c4d45c578bb7d0136365"
    f5_url = (
        "https://api.the-odds-api.com/v4/sports/baseball_mlb/odds/"
        "?apiKey=" + ODDS_KEY +
        "&regions=us&markets=h2h_h1&oddsFormat=decimal&dateFormat=iso"
    )
    f5_r = requests.get(f5_url, timeout=10)
    print(f"   F5 odds status: {f5_r.status_code}")
    if f5_r.ok:
        f5_data = f5_r.json()
        for g in f5_data:
            ...
            if not home or not books: continue
                ...
                for mkt in bm.get('markets',[]):
                    if mkt.get('key') == 'h2h_h1':
                        for oc in mkt.get('outcomes',[]):
                            ...
                            if oc.get('name') == home and p > 0:
                                home_odds_list.append(p)
                            elif oc.get('name') == away and p > 0:
                                away_odds_list.append(p)
            
            if not home_odds_list or not away_odds_list: continue
            avg_h = sum(home_odds_list)/len(home_odds_list)
            avg_a = sum(away_odds_list)/len(away_odds_list)
            hp = 1/avg_h; ap = 1/avg_a; tot = hp+ap
            
            f5_odds.append({
                'home': home, 'away': away,
                'date': (g.get('commence_time','')[:10]),
                'f5_home_odds':    round(avg_h,3),
                'f5_away_odds':    round(avg_a,3),
                'f5_home_impl':    round(hp/tot*100) if tot>0 else 50,
                'f5_away_impl':    round(ap/tot*100) if tot>0 else 50,
                'f5_books_count':  len(books),
            })
        print(f"   f5_odds: {len(f5_odds)} games")
    else:
        print(f"   F5 odds failed: {f5_r.status_code}")
except Exception as e:
    print(f"   f5_odds error: {e}")
    f5_odds = []

# ════════════════════════════════════════════════════════
# BETTING SPLITS — TheOddsAPI consensus
# ════════════════════════════════════════════════════════
print("-> Betting splits (TheOddsAPI)...")
betting_splits = []
try:
    ODDS_KEY = "27efeb9368c8c4d45c578bb7d0136365"
    odds_url = (
    ...
    odds_r = requests.get(odds_url, timeout=10)
    print(f"   TheOddsAPI: {odds_r.status_code}")
    if odds_r.ok:
        ...
        for g in odds_data:
            ...
            avg_a = sum(away_odds_list)/len(away_odds_list)
            hp = 1/avg_h; ap = 1/avg_a; tot = hp+ap
            betting_splits.append({
                'home': home, 'away': away,
                'date': (g.get('commence_time','')[:10]),
                'ml_home_pct_bets': None, 'ml_away_pct_bets': None,
                'ml_home_pct_money': None, 'ml_away_pct_money': None,
                'over_pct_bets': None, 'total_line': None,
                'sharp_signal': None,
                'home_implied_prob': round(hp/tot*100) if tot>0 else 50,
                'away_implied_prob': round(ap/tot*100) if tot>0 else 50,
                'books_count': len(books),
            })
        print(f"   betting_splits: {len(betting_splits)} games")
    else:
        print(f"   TheOddsAPI failed: {odds_r.status_code}")
except Exception as e:
...
output = {
    ...
    "pitcher_l5":   {str(k):v for k,v in pitcher_l5.items()},
    "f5_odds":      f5_odds,
}
...
print(f"   betting_splits:{len(betting_splits)} games")
print(f"   pitcher_l5:{len(pitcher_l5)} pitchers | f5_odds:{len(f5_odds)} games")
