...
try:
    ...
    # Try sportsbettingdime public betting page
    sbd_url = "https://www.sportsbettingdime.com/mlb/public-betting-trends/"
    sbd_r = requests.get(sbd_url, headers=HEADERS_SCRAPE, timeout=15)
    print(f"   SBD status: {sbd_r.status_code}")

    if sbd_r.ok:
        soup = BeautifulSoup(sbd_r.text, 'html.parser')
        # Find game rows — SBD uses table rows with team data
        ...
        for row in rows:
            ...
            if len(cells) >= 4:
                ...
                pcts = [pct.search(c) for c in text_cells]
                if any(pcts):
                    print(f"   Row data: {text_cells[:6]}")

    # Try covers.com consensus API
    covers_url = f"https://contests.covers.com/consensus/topconsensus/mlb/overall"
    covers_headers = {
        'User-Agent': 'Mozilla/5.0',
        'Accept': 'application/json, text/javascript, */*',
        'X-Requested-With': 'XMLHttpRequest',
        'Referer': 'https://www.covers.com/sport/baseball/mlb/betting-splits',
    }
    time.sleep(1)
    cov_r = requests.get(covers_url, headers=covers_headers, timeout=15)
    print(f"   Covers status: {cov_r.status_code}")
    if cov_r.ok:
        try:
            cov_data = cov_r.json()
            print(f"   Covers keys: {list(cov_data.keys())[:5] if isinstance(cov_data, dict) else type(cov_data)}")
            print(f"   Covers sample: {str(cov_data)[:300]}")
        except: print(f"   Covers response: {cov_r.text[:200]}")

    # Try ActionNetwork new endpoint format
    date_iso = TODAY.strftime('%Y-%m-%d')
    an_new_url = f"https://api.actionnetwork.com/web/v1/scoreboard/mlb?period=full&bookIds=15,30,76&date={date_iso}"
    an_headers = {
        'User-Agent': 'Mozilla/5.0',
        'Accept': 'application/json',
    ...
    time.sleep(1)
    an_r = requests.get(an_new_url, headers=an_headers, timeout=15)
    print(f"   ActionNetwork new: {an_r.status_code} → {an_r.text[:200]}")

    print(f"   betting_splits: {len(betting_splits)} games (debug run)")
